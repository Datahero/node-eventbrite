...
eventbriteAPI_v3.prototype.create_event = function (params, callback) {
  var availableParams = [
    // "name", //The name of the event. Value cannot be empty nor whitespace.
    // "description", //The description on the event page
    // "organizer_id", //The ID of the organizer of this event
    // "start", //{utc: The start time of the event, timezone: Start time timezone (Olson format)}
    // "end", //{utc: The end time of the event, timezone: End time timezone (Olson format)}
    // "hide_start_date", //Whether the start date should be hidden
    // "hide_end_date", //Whether the end date should be hidden
    // "currency", //Event currency (3 letter code)
    // "venue_id", //The ID of a previously-created venue to associate with this event. You can omit this field or set it to null if you set online_event.
    // "online_event", //Is the event online-only (no venue)?
    // "listed", //If the event is publicly listed and searchable. Defaults to True.
    // "logo_id", //The logo for the event
    // "category_id", //The category (vertical) of the event
    // "subcategory_id", //The subcategory of the event (US only)
    // "format_id", //The format (general type) of the event
    // "shareable", //If users can share the event on social media
    // "invite_only", //Only invited users can see the event page
    // "password", //Password needed to see the event in unlisted mode
    // "capacity", //	Set specific capacity (if omitted, sums ticket capacities)
    // "show_remaining", //If the remaining number of tickets is publicly visible on the event page
    // "source", //	Source of the event (defaults to API)
    "event"
  ];

  if (!params.event) {
    return callback("This request requires an event", null)
  } else if (!params.event.name) {
    return callback("This request requires an name", null)
  } else if (!params.event.start) {
    return callback("This request requires an start", null)
  } else if (!params.event.end) {
    return callback("This request requires an end", null)
  } else if (!params.event.currency) {
    return callback("This request requires an currency", null)
